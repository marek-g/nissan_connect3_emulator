...
/// to page size.
fn mem_align_down(address: u32, alignment: Option<u32>) -> u32 {
...
/// If `alignment` is not specified the `address` will be aligned
/// to page size.
fn mem_align_up(address: u32, alignment: Option<u32>) -> u32 {
    let align = alignment.unwrap_or(0x1000);
    (address / align + 1) * align
}

fn to_unicorn_permissions(perms: program::Flags) -> Permission {
...
fn push_text_on_stack(unicorn: &mut Unicorn<Context>, address: u32, text: &str) -> u32 {
